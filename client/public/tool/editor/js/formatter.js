...
function formatHTML(code) {
  try {
    ...
    const tokens =
      code.match(
        /<!--[\s\S]*?-->|<(pre|textarea|script|style|code)[\s\S]*?<\/\1>|<[^>]+>|[^<]+/gi
      ) || [];
    ...
    tokens.forEach(token => {
      ...
      const isPreserveBlock =
        /^<(pre|textarea|script|style|code)[\s\S]*?<\/\1>/i.test(trimmed);
      if (isPreserveBlock) {
      ...
      if (/^<\/[^>]+>/.test(trimmed)) {
        ...
        /^<!--/.test(trimmed) ||
        /^<[^>]+\/>$/.test(trimmed) ||
        (trimmed.startsWith("<") && voidTags.test(trimmed.slice(1)))
      ) {
        formatted += tab.repeat(indent) + trimmed + "\n";
