...
class AutoCropEngine {
  ...
  getTargetBgColor(imgData, width, height) {
    ...
    if (mode === "custom_color")
      return this.hexToRgb(this.config.customBgColor);
    if (mode === "auto_corner")
      return this.detectCornerColor(imgData, width, height);
  ...
  async analyze(userConfig = {}) {
    ...
    const colorTolSq =
      this.config.colorTolerance * this.config.colorTolerance * 3;
    ...
    this.processedBoxes = SmartProcessor.processBoxes(
      rawBoxes,
      this.config,
      width,
      height
  ...
  generateSingleCropThumbnail(box) {
    ...
    const rawW = Math.max(
      1,
      Math.min(this.width - x, box.maxX - box.minX + 1 + pad * 2)
    );
    const rawH = Math.max(
      1,
      Math.min(this.height - y, box.maxY - box.minY + 1 + pad * 2)
    );
  ...
  mergeSelectedBoxes(selectedIds) {
    ...
    this.processedBoxes = this.processedBoxes.filter(
