...
import { ArrowUpRight, Check, ChevronDown, Copy, Facebook, Instagram, Linkedin, Mail, Menu, Phone, Share2, Sparkles, X } from "lucide-react";
...
export default function Home() {
  ...
  const shareProject = async (projectName: string) => {
    const shareUrl = `${window.location.origin}${window.location.pathname}#work`;
    const shareData = { title: `${projectName} — WebCraft Studio`, text: `ชมผลงาน ${projectName} จาก WebCraft Studio`, url: shareUrl };
    try {
      if (navigator.share) {
      ...
      toast.success("คัดลอกลิงก์ผลงานแล้ว");
    } catch (error) {
      if ((error as DOMException)?.name !== "AbortError") toast.error("ยังแชร์ไม่ได้ ลองใหม่อีกครั้ง");
    }
  };
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-paper text-ink selection:bg-cobalt selection:text-white">
      ...
      <section id="work" className="bg-bluewash py-24 sm:py-32"><div className="container"><div className="mb-14 flex flex-col justify-between gap-6 sm:flex-row sm:items-end"><div><p className="eyebrow mb-5"><span className="eyebrow-dot" /> SELECTED WORK / 2024—26</p><h2 className="font-display text-5xl font-bold tracking-[-0.07em] sm:text-7xl">งานที่เล่า<br /><span className="text-cobalt">เรื่องได้ดี</span></h2></div><p className="max-w-[260px] text-sm leading-6 text-ink/60">ผลงานแต่ละชิ้นถูกออกแบบให้เหมาะกับเสียงของแบรนด์ ไม่ใช่แค่สวยในหน้าจอ</p></div><div className="grid gap-7 lg:grid-cols-[1.15fr_0.85fr]"><article className="group"><div className="work-image aspect-[4/3]"><img src="/manus-storage/webcraft-work-1_558a4eaf.png" alt="ตัวอย่างงานออกแบบเว็บไซต์ร้านค้าออนไลน์" /><span className="work-label">01 / E-COMMERCE</span><div className="work-actions"><button aria-label="แชร์ Morrow Objects" onClick={() => shareProject("Morrow Objects")}><Share2 size={15} /> แชร์</button><a aria-label="แชร์ Morrow Objects ไป Facebook" href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(`${window.location.origin}${window.location.pathname}#work`)}`} target="_blank" rel="noreferrer"><Facebook size={15} /></a><a aria-label="แชร์ Morrow Objects ไป X" href={`https://twitter.com/intent/tweet?text=${encodeURIComponent("ชมผลงาน Morrow Objects จาก WebCraft Studio")}&url=${encodeURIComponent(`${window.location.origin}${window.location.pathname}#work`)}`} target="_blank" rel="noreferrer"><X size={15} /></a></div></div><div className="mt-5 flex items-start justify-between gap-4"><div><h3 className="font-display text-2xl font-bold">Morrow Objects</h3><p className="mt-1 text-sm text-ink/55">ร้านของแต่งบ้านที่เล่าเรื่องวัสดุอย่างมีรสนิยม</p></div><ArrowUpRight className="mt-1 transition-transform group-hover:-translate-y-1 group-hover:translate-x-1" /></div></article><article className="group lg:mt-24"><div className="work-image aspect-[4/3]"><img src="/manus-storage/webcraft-work-2_75f57a21.png" alt="ตัวอย่างงานออกแบบเว็บไซต์ธุรกิจโรงแรม" /><span className="work-label">02 / HOSPITALITY</span><div className="work-actions"><button aria-label="แชร์ Serein House" onClick={() => shareProject("Serein House")}><Share2 size={15} /> แชร์</button><a aria-label="แชร์ Serein House ไป LinkedIn" href={`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(`${window.location.origin}${window.location.pathname}#work`)}`} target="_blank" rel="noreferrer"><Linkedin size={15} /></a><button aria-label="คัดลอกลิงก์ Serein House" onClick={() => shareProject("Serein House")}><Copy size={15} /></button></div></div><div className="mt-5 flex items-start justify-between gap-4"><div><h3 className="font-display text-2xl font-bold">Serein House</h3><p className="mt-1 text-sm text-ink/55">พื้นที่พักใจที่เริ่มต้นตั้งแต่หน้าแรก</p></div><ArrowUpRight className="mt-1 transition-transform group-hover:-translate-y-1 group-hover:translate-x-1" /></div></article></div></div></section>
