...
const TRANSLATIONS = {
  th: {
    ...
    search_placeholder: "ค้นหาเครื่องมือ เช่น Merge, Convert, OCR, AI, Live Editor...",
    ...
    hero_subtitle: "แปลงไฟล์, รวมไฟล์, แยกหน้า, บีบอัด, เซ็นชื่อ, แก้ไข และฟีเจอร์ AI ครบครันในที่เดียว",
    tab_all: "ทั้งหมด",
    ...
    workflow_desc: "สร้างขั้นตอนการทำงานแบบกำหนดเองด้วยเครื่องมือโปรดของคุณ เพื่อประหยัดเวลาและใช้งานซ้ำได้ทุกเมื่อ",
    workflow_link: "สร้างเวิร์กโฟลว์ ↗",
    editor_title: "Live HTML Editor + Inspector",
    editor_desc: "แก้ไขและตรวจสอบโค้ด HTML สดแบบ Real-time พร้อมฟังก์ชันจัดเก็บไฟล์และเชื่อมต่อ Google Drive",
    editor_link: "เปิด Live Editor ↗",
    cropper_title: "Smart Image Cropper & Auto Sprite Extractor",
    cropper_desc: "ตัดภาพแยกตัวละครอัตโนมัติ (Auto-Cut On Drop), ตรวจจับพื้นหลังสีทึบ/Alpha, ปรับสเกลภาพ Pixel Art และส่งออก ZIP",
    ...
    toast_file_selected: (name, size) => `📁 เลือกไฟล์ "${name}" เรียบร้อยแล้ว (${size} MB)`,
  ...
  en: {
    ...
    search_placeholder: "Search tools e.g. Merge, Convert, OCR, AI, Live Editor...",
    ...
    hero_subtitle: "Convert, merge, split, compress, sign, edit, and use AI features all in one place.",
    tab_all: "All",
    ...
    workflow_desc: "Create custom workflows with your favorite tools, automate tasks, and reuse them anytime.",
    workflow_link: "Create workflow ↗",
    editor_title: "Live HTML Editor + Inspector",
    editor_desc: "Live edit & inspect HTML elements in real-time with file manager & Google Drive integration.",
    editor_link: "Open Live Editor ↗",
    cropper_title: "Smart Image Cropper & Auto Sprite Extractor",
    cropper_desc: "Smart auto crop & sprite extraction on drop, alpha/solid background removal, pixel art scaling, and batch ZIP export.",
    ...
    toast_file_size_error: "⚠️ File size exceeds 100 MB. Please choose a smaller file.",
    toast_file_selected: (name, size) => `📁 Selected "${name}" successfully (${size} MB)`,
    toast_process_success: (title) => `⚡ Demo Mode: Successfully processed ${title}!`
  }
};
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function updateStaticTexts(lang) {
    ...
    if (btnStart && !btnStart.disabled) btnStart.textContent = t.btn_start_process;

    // Active tool modal update
    if (currentActiveTool) {
      const title = currentActiveTool.title ? (currentActiveTool.title[lang] || currentActiveTool.title.en || currentActiveTool.title) : '';
      const desc = currentActiveTool.desc ? (currentActiveTool.desc[lang] || currentActiveTool.desc.en || currentActiveTool.desc) : '';
  ...
  function renderTools() {
    ...
    TOOLS_DATA.forEach(tool => {
      ...
      const titleTh = typeof tool.title === 'object' ? (tool.title.th || '') : tool.title;
      const titleEn = typeof tool.title === 'object' ? (tool.title.en || '') : tool.title;
      const descTh = typeof tool.desc === 'object' ? (tool.desc.th || '') : tool.desc;
      const descEn = typeof tool.desc === 'object' ? (tool.desc.en || '') : tool.desc;
      ...
      card.dataset.search = `${tool.name || ''} ${titleTh} ${titleEn} ${descTh} ${descEn}`.toLowerCase();
      card.dataset.tool = activeTitle;

      let badgeHTML = tool.isNew ? `<span class="badge-new">${t.badge_new}</span>` : '';
      ...
      card.addEventListener('click', (e) => {
        if (e.target.closest('.workflow-link, .editor-link, .cropper-link')) return;
    ...
    workflowCard.dataset.search = 'workflow automate custom เวิร์กโฟลว์ อัตโนมัติ auto flows create';
    ...
    editorCard.dataset.search = 'live html editor inspector inspect code drive google web ตัวแก้ไขโค้ด ตรวจสอบองค์ประกอบ';
    ...
    cropperCard.dataset.search = 'image cropper crop sprite extractor character cut ตัดภาพ ตัดรูปภาพ แยกตัวละคร สไปรท์ คัดลอกภาพ ตัดภาพอัตโนมัติ pixel art';
  ...
  function applyCurrentSearchAndFilter() {
    ...
    cards.forEach(card => {
      const isSpecialCard = card.classList.contains('workflow-card') || card.classList.contains('editor-card') || card.classList.contains('cropper-card');
      const cardCategory = card.dataset.category || 'all';
      const searchIndex = card.dataset.search || '';

      const matchCategory = (currentCategory === 'all' || cardCategory === currentCategory || (isSpecialCard && currentCategory === 'all'));
  ...
  function showToast(msg) {
    ...
    toastTimeout = setTimeout(() => { toastMsg.style.display = 'none'; }, 2800);
  }
  ...
  function handleFiles(files) {
    ...
    showToast(t.toast_file_selected(file.name, (file.size / (1024 * 1024)).toFixed(2)));
  }
