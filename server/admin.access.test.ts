...
const {
  createLeadMock,
  upsertContentSettingMock,
  createMediaAssetMock,
  storagePutMock,
} = vi.hoisted(() => ({
  createLeadMock: vi.fn(async () => 42),
  upsertContentSettingMock: vi.fn(async () => 7),
  createMediaAssetMock: vi.fn(async () => 9),
  storagePutMock: vi.fn(async () => ({
    key: "content/1/hero/hero.png",
    url: "/media/content/1/hero/hero.png",
  })),
}));
vi.mock("./db", () => ({
...
describe("lead workflow", () => {
  ...
  it("rejects an empty lead name before touching the database", async () => {
    const caller = appRouter.createCaller(makeContext("user"));
    await expect(
      caller.leads.create({ name: "", contact: "contact" })
  ...
  it("rejects unsupported content keys and empty values", async () => {
    const caller = appRouter.createCaller(makeContext("admin"));
    await expect(
      caller.content.upsert({
        contentKey: "nav" as never,
        language: "th",
        value: "ข้อความ",
      })
    ).rejects.toMatchObject({ code: "BAD_REQUEST" });
  ...
  it("saves an allowed content key for an admin", async () => {
    ...
    expect(upsertContentSettingMock).toHaveBeenCalledWith(
      expect.objectContaining({
        contentKey: "heroTitle",
        language: "th",
        value: "ข้อความจริง",
  ...
  it("uploads a valid image and persists its storage reference", async () => {
    ...
    expect(createMediaAssetMock).toHaveBeenCalledWith(
      expect.objectContaining({
        slot: "hero",
        storageKey: "content/1/hero/hero.png",
        fileSize: 1,
        uploadedBy: 1,
      })
    );
